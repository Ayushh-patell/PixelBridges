...
const ExpertiseScreen = () => {
  ...
  useEffect(() => {
    if(window.innerWidth>768) {
      ...
      if(isLeftOn) {
        gsap.to("#leftCarousel", {left:0, opacity:1, duration:1, ease:'none'})
        gsap.to("#rightCarousel", {right:"-100%" ,opacity:0, duration:1, ease:'none'})
      }
      else {
        gsap.to("#leftCarousel", {left:"-100%", opacity:0, duration:1, ease:'none'})
        gsap.to("#rightCarousel", {right:0, opacity:1, duration:1, ease:'none'})
      }
    }
...
<button onClick={() => setisLeftOn((prev) => !prev)} className={` md:h-[14px] h-2 lg1:w-1/2 md:w-2/3 w-4/5 rounded-full bg-[#d9d9d9]/10 flex justify-start items-center mb-12 mx-auto`}>
          <div className={` w-1/2 rounded-full h-full ${isLeftOn?" bg-[#B896FF] translate-x-0":" bg-[#5E81FF] md:translate-x-32 translate-x-12"} transition-all duration-700`}></div>
        </button>
        ...
        <div className=' text-center xl1:text-xl lg1:text-lg text-sm space-y-3 px bg-[#110E17]-4'>
            <p>
...
<button onClick={() => setisLeftOn((prev) => !prev)} className={` md:h-[14px] h-2 lg1:w-1/2 md:w-2/3 w-4/5 rounded-full bg-[#d9d9d9]/10 flex justify-end items-center mx-auto mb-10`}>
        <div className={` w-1/2 rounded-full h-full ${!isLeftOn?" bg-[#5E81FF] translate-x-0":" bg-mainBg md:-translate-x-32 -translate-x-12"} transition-all duration-700`}></div>
        </button>
