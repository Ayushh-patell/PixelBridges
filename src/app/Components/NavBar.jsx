...
const NavBar = () => {
    ...
    useEffect(() => {
      if(currTab === 0) {
        ...
        if(currTab === 1) {
            ...
            gsap.to("#opacityTopBar", {opacity:0})
            gsap.to("#lightTopBar", {opacity:0})
            if(document.getElementById("ExpertiseScreen").getAttribute("data-isleft") === "1") {
          ...
          if(currTab === 2) {
            ...
            gsap.to("#opacityTopBar", {opacity:0.6})
            gsap.to("#lightTopBar", {opacity:0})
            gsap.to("#TopBar", {opacity:0})
        }
    },[currTab])
  ...
  return (
    ...
    <Image src={topBar2} id='lightTopBar' aria-hidden alt='top bar' className=' pointer-events-none fixed z-[-4] xl:w-[45vw] lg:w-[50vw] md:w-[60vw] sm3:w-[75vw] w-[90vw] object-contain object-right-top top-0 right-0 opacity-0'/>
    <Image src={topBar} id='opacityTopBar' aria-hidden alt='top bar' className=' pointer-events-none fixed z-[-4] xl:w-[45vw] lg:w-[50vw] md:w-[60vw] sm3:w-[75vw] w-[90vw] object-contain object-right-top top-0 right-0 opacity-0'/>
      <nav className={` flex justify-between items-center 1xl:gap-20 lg:gap-14 gap-8 lg:text-lg md:text-base sm1:text-sm sm4:text-xs text-[10px] ${mont.className}`}>
        <button onClick={() => setcurrTab(0)} className={` px-1 pb-0 font-bold border-b-4 ${currTab===0? " border-mainBg text-white":" border-transparent text-white/40"} `}>Home</button>
        <button onClick={() => setcurrTab(1)} className={` px-1 pb-0 font-bold border-b-4 ${currTab===1? " border-mainBg text-white":" border-transparent text-white/40"} `}>Expertise</button>
        <button onClick={() => setcurrTab(2)} className={` px-1 pb-0 font-bold border-b-4 ${currTab===2? " border-mainBg text-white":" border-transparent text-white/40"} `}>About</button>
      </nav>
